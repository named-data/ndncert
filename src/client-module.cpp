...
ClientModule::generateProbeInterest(const ClientCaItem& ca, const std::string& probeInfo)
{
  ...
  interest->setParameters(paramFromJson(paramJson));
                                  ...
                                  const Name& identityName)
{
  ...
  interest->setMustBeFresh(true);
  interest->setCanBePrefix(false);
  interest->setParameters(paramFromJson(genNewRequestJson(m_ecdh.getBase64PubKey(), certRequest)));
...
ClientModule::generateChallengeInterest(const JsonSection& paramJson)
{
  ...
  auto payload = ss.str();
  auto paramBlock = genEncBlock(tlv::Parameters, m_ecdh.context->sharedSecret, m_ecdh.context->sharedSecretLen,
                                (const uint8_t*)payload.c_str(), payload.size());
  interest->setParameters(paramBlock);
...
ClientModule::paramFromJson(const JsonSection& json)
{
  ...
  return makeStringBlock(ndn::tlv::Parameters, ss.str());
}
